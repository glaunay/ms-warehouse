...
*/
function testRequest(query, nameDB, accountName, passwordDB, addressDB, portDB, proxyBool) {
    let reqEmitter = new EventEmitter();
    let chunkRes = '';
    let chunkError = '';
    let curl;
    if (proxyBool) {
        curl = child_process_1.spawn('curl', ['--noproxy', `${addressDB}`, '-s', '-S', '-H', 'Content-Type:application/json', '-H', 'charset=utf-8', '-d', `${JSON.stringify(query)}`, '-X', 'POST', `http://${accountName}:${passwordDB}@${addressDB}:${portDB}/${nameDB}/_find`]);
    }
    else {
        curl = child_process_1.spawn('curl', ['-s', '-S', '-H', 'Content-Type:application/json', '-H', 'charset=utf-8', '-d', `${JSON.stringify(query)}`, '-X', 'POST', `http://${accountName}:${passwordDB}@${addressDB}:${portDB}/${nameDB}/_find`]);
    }
...
*/
function addToDB(data, nameDB, accountName, passwordDB, addressDB, portDB, proxyBool) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let elem of arrayData) {
            let addObj = new addData(elem, nameDB, accountName, passwordDB, addressDB, portDB, proxyBool);
...
class addData extends EventEmitter {
    constructor(_docList, _nameDB, _accountName, _passwordDB, _addressDB, _portDB, _proxyBool) {
        super();
    ...
    _curl() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let p = new Promise((resolve, reject) => {
                ...
                // (1)
                if (self.proxyBool) {
                    curl = child_process_1.spawn('curl', ['--noproxy', `${self.addressDB}`, '-s', '-S', '-H', 'Content-Type:application/json', '-d', `{"docs": ${JSON.stringify(self.docList)}}`, '-X', 'POST', `http://${self.accountName}:${self.passwordDB}@${self.addressDB}:${self.portDB}/${self.nameDB}/_bulk_docs`]);
                }
                else {
                    curl = child_process_1.spawn('curl', ['-s', '-S', '-H', 'Content-Type:application/json', '-d', `{"docs": ${JSON.stringify(self.docList)}}`, '-X', 'POST', `http://${self.accountName}:${self.passwordDB}@${self.addressDB}:${self.portDB}/${self.nameDB}/_bulk_docs`]);
                }
                // (2)
                curl.stdout.on('data', (data) => {
