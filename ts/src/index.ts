...
if (types.isServerConfig(configContent)) {
	logger.log('info', `Server config file perfectly loaded`)
	...
	logger.log('error', `Config server file is not in a right format, please refer to the documentation`);
	throw 'stop execution';
...
emitter.on('created', () => {
	...
	runOptions().then(() => {
		...
		server.startServerSocket(portSocket).on('findBySocket', (packet : any) => {
			...
			.on('socketSucceed', (docsArray : types.stringMap[]) => {
				packet.data(docsArray);
				server.push('found', packet = packet);
			})
			.on('socketNoResults', (docsArray : types.stringMap[]) => {
				packet.data(docsArray);
